...
const NavItem = (props) => {
  ...
  return(
    <li className="nav-item mt-2 " role="presentation">
      <Link 
      id={props.id} 
      className={ (props.size ? reducedNav: "") + (props.state === props.id ? styleSelected : styleUnSelected)} 
      to={`${props.url}`} 
      onClick={() => props.setState(props.id)}
      >
      {
        props.icon ? <FontAwesomeIcon className={ props.size ? "text-align-center" : "" } icon={props.icon}/> : null
      }
        <span className={ props.size ? "h6" :"pl-2 h5"}>{props.title}</span>
      </Link>
      {props.children}
    </li>
  )
